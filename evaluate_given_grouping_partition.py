...
def usage():
    print "<input results file> <input parition files>"
...
def main():
    ...
    input_peptide_list_filenames = []
    input_peptide_list_filenames.append(sys.argv[2])
    input_peptide_list_filenames.append(sys.argv[3])

    products_to_rt_map = parse_identification_file(input_results_filename)

    all_peptides = []
    partitioned_peptide_list = []
    for input_peptide_list_filename in input_peptide_list_filenames:
        ...
        partitioned_peptide_list.append(table_data["Peptides"])
